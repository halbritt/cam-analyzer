...
| D019 | accepted | VERIFIED (partial) | **Honest visualization projection and rendering grammar.** Charts consume a source-blind JSON projection plus a provenance/refusal rendering grammar; renderers may draw, segment, or style the projection but may not recompute source facts or upgrade provenance. | RFC 0004 turns "pretty charts" into a boundary-preserving projection problem: every plotted value carries unit/frame/provenance or a refusal, so chart output cannot launder cam-card extrapolation into measured-looking curves. | Built: `analysis/projection.py` samples only `CamProfile` C5 queries; `visualization/grammar.py` splits sampled curves at `ProvenanceMap` boundaries and emits no-line `UNDECIDABLE` segments across refused samples; `cam-analyze --charts json` exposes the static projection. Witness: `tests/test_visualization_projection.py`, `tests/test_visualization_grammar.py`, `tests/test_cli.py`, and `tests/test_architecture_boundary.py`. **Not built:** SVG/ECharts adapter, crop-proof ledger, uncertainty-band calibration, PTV collision chart, "go measure THIS", and web UI. | A renderer adapter starts consuming the projection; or a chart needs a fact not expressible through `CamProfile`. |
