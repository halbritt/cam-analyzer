...
| Lift reconstruction + velocity/acceleration/jerk | **VERIFIED** (with Nyquist refusal) | `tests/test_canonical_profile.py`; `tests/test_approximate_derivatives.py` | |
...
| Reporting | **VERIFIED** (Markdown only) | `render_markdown_report` (`analysis/reporting.py`); `tests/test_safety_and_reporting.py`, `tests/test_cli.py` | **HTML / PDF are `DESIGNED`, not built.** |
| `cam-analyze` CLI (`--reference`, card file) | **VERIFIED** | `tests/test_cli.py::test_main_with_reference_flag_prints_report`, `::test_main_with_card_file`, `::test_main_incoherent_card_is_refused`; entry point `pyproject.toml:20` | |
