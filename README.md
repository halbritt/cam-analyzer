...
| Reporting | Markdown summary with lift-threshold durations, quality warnings, and an install checklist; static RFC-0004 chart-projection JSON via `cam-analyze --charts json`; full-cycle overlap-centered -360° to +360° static SVAJ SVG with confidence bands, hard event markers, threshold lines, summary panel, and secondary canonical 0° to 720° overview via `cam-analyze --charts svg` (HTML / PDF / ECharts / webapp are `DESIGNED`, not built) |
